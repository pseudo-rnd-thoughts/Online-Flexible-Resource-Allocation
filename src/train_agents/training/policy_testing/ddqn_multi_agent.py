...
if __name__ == "__main__":
    ...
    task_pricing_agents = [
        TaskPricingDdqnAgent(agent_num, DqnLstmNetwork(agent_num, 9, 10), save_folder=folder)
        for agent_num in range(3)
    ]
    resource_weighting_agents = [
        ResourceWeightingDdqnAgent(agent_num, DqnLstmNetwork(agent_num, 10, 10), save_folder=folder)
        for agent_num in range(3)
    ]
