...
if __name__ == "__main__":
    ...
    task_pricing_ddqn_agents = [
        TaskPricingDdqnAgent(agent_num, DqnLstmNetwork(agent_num, 9, 10), save_frequency=25000, save_folder=folder,
    ...
    resource_weighting_ddqn_agents = [
        ResourceWeightingDdqnAgent(agent_num, DqnLstmNetwork(agent_num, 10, 10), save_frequency=25000, save_folder=folder,
                                   replay_buffer_length=50000, training_replay_start_size=500,
                                   target_update_frequency=100, final_exploration_frame=100000)
        for agent_num in range(3)
    ]
